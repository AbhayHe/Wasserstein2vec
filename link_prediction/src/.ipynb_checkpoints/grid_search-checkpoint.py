...
def load_or_create_graph(filename, args):
    cache_file = f"graph/results/{args.dataset}/{args.dataset}_cache.pkl"
...
def grid_search(G, args):
    ...
    param_grid = {
        ...
        # 'p': [0.25,0.5,1, 2],
        # 'q': [0.25,0.5,1, 2],
        'degree_weight': [0,1],  # 0: 不使用度数特征, 1: 使用度数特征
        'edge_weight': [0,1],     # 0: 不使用边权重特征, 1: 使用边权重特征
        'clustering_weight': [0,1] # 0: 不使用聚类系数特征, 1: 使用聚类系数特征
    ...
    # 加载或创建结果文件
        result_file = os.path.join(f'graph/results/{args.dataset}/Ablation/', 'grid_search_results_{}.json'.format(args.modelname))
        ...
        try:
            ...
            # 构建嵌入文件名
            embs_file = (f"graph/results/{args.dataset}/Ablation/"
                        f"{args.dataset}_grid_search_{idx}_"
                        f"{params['degree_weight']}_{params['edge_weight']}_"
                        f"{params['clustering_weight']}_{args.modelname}.pkl")
            ...
            if os.path.exists(embs_file):
                ...
                if args.modelname == 'WassersteinWalker':
                    from Wasserstein2vevc import Wasserstein2vec
                    embs = Wasserstein2vec(G, current_args)
                elif args.modelname == 'hyper2vec':
                    from hyper2vec import hyper2vec
                    embs = hyper2vec(G, current_args)
                else:
                    raise ValueError(f"Invalid model name: {args.modelname}")
            ...
            if mrr > best_mrr:
                ...
                # # 保存最佳嵌入
                # best_embs_file = (f"graph/results/{args.dataset}/Ablation/"
                #                 f"{args.dataset}_best_embeddings.pkl")
...
def main(args):
    ...
    args.modelname = 'WassersteinWalker'
    print('\n##### reading hypergraph...')
    filename = f"graph/{dataset}/edgelist_filtered.txt"
