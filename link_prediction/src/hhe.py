...
def train_nhe(G, args):
    ...
    # 创建数据加载器
    train_dataset = EdgeDataset(G, data_dir=f"graph/{args.dataset}/new_network.edgelist")
    ...
    test_dataset = EdgeDataset(G, data_dir=f"graph/{args.dataset}/new_valid.edgelist")
    ...
    # 保存训练历史
    history_file = os.path.join(save_dir, 'hhe_training_history.json')
...
if __name__ == "__main__":
    ...
    args.dimensions = 64
    args.batch_size = 256
    for dataset in ['jingrong']:
        args.dataset = dataset
        # 读取超图
        print(f"开始训练{args.dataset}模型")
        G = read_graph(f"graph/{args.dataset}/new_network.edgelist", 'hhe2vec')
    ...
    # imdb edgelist.txt,valid.edgelist 128
    # dblp edgelist_new.txt,valid.edgelist 128
    # FB-AUTO new_network.txt,valid.edgelist  128
